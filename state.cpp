        ...
        for (int y=-xb; y<=xb; ++y) {
          for (int z=-xc; z<=xc; ++z) {
            ...
            for (int ifrag=0; ifrag<nfragments; ++ifrag){ 
              ...
              fprintf(fs, " $CONTRL ");
              if (jobtype == RUN_ENERGY) 
                fprintf(fs, "RUNTYP=ENERGY ");
              else if (jobtype == RUN_FORCE || jobtype == RUN_MOLDYN)
                fprintf(fs, "RUNTYP=GRADIENT ");
              fprintf(fs,"SCFTYP=RHF ");
              if (ifrag == chgfrag) {
                fprintf(fs, "ICHARG=1 ");
              } else {
                fprintf(fs, "ICHARG=0 ");
	      ...
	      //}
 	      fprintf(fs, "ISPHER=1 ");
              ...
              fprintf(fs, " $BASIS ");
              fprintf(fs, "GBASIS=CCD "); // current implementation limited to cc-pVDZ basis set
              fprintf(fs, "$END\n");
              // BUGBUGBUGBUGBUGBUGBUGBUGBUGBUGBUGBUGBUGBUGBUGBUG
              ...
              // 
	      fprintf(fs, " $SYSTEM ");
              fprintf(fs, "MWORDS=50 ");
              fprintf(fs, "MEMDDI=1000 ");
              fprintf(fs, "$END\n");
                  ...
                  for (int z0=-cfield; z0<=cfield; ++z0) {
                    ...
                    for (int iatom=0; iatom<natoms; ++iatom) {
                      if (atom->fragment[istate*natoms + iatom] != ifrag || x0 != x || y0 != y || z0 != z) {
                        double mmq = atom->getCharge(iatom, istate);
                        //fprintf(fs, "%20.10lf %20.10lf %20.10lf %16.4lf 0.12446\n",
                        fprintf(fs, "  BQ %20.10lf %20.10lf %20.10lf %16.4lf\n",
                                atom->coord[3*iatom] + x0*cellA,
                                atom->coord[3*iatom+1] + y0*cellB,
                                atom->coord[3*iatom+2] + z0*cellC,
                                mmq
